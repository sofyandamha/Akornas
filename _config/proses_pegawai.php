...
if (isset($_GET['add'])) {
    ...
    if (in_array($tipe, $ekstensi) === true) {
        if ($ukuranFile < 1048576) { //1 mb
            ...
            create("INSERT INTO pegawai VALUES ('$nip','$nik','$nama_pegawai','$foto','$tempat_lahir','$tgl_lahir','$jk','$no_hp','$agama','$email','$alamat','$goldarah','$stat_nikah','$stat_pegawai','aktif','$jabatan','$struktural','$tahun_pensiun')");
    ...
    if ($_FILES['foto']['name'] == '') {
        $query = mysqli_query($koneksi, "UPDATE pegawai SET nip='$nip',nik='$nip',nama_pegawai='$nama_pegawai', tempat_lahir='$tempat_lahir', tanggal_lahir='$tgl_lahir', jenis_kelamin='$jk', no_hp='$no_hp', agama='$agama', email='$email',alamat='$alamat',gol_darah='$goldarah',status_pernikahan='$stat_nikah', status_kepegawaian='$stat_pegawai', status_user='$stat_user', jabatan='$jabatan', struktural='$struktural', tahun_pensiun='$tahun_pensiun' WHERE nip='$nipAsli' ");
        if ($query) {
            echo '<script>
        ...
        if (in_array($tipe, $ekstensi) === true) {
            if ($ukuranFile < 1048576) { //1 mb
                ...
                mysqli_query($koneksi, "UPDATE pegawai SET nip='$nip',nik='$nip',nama_pegawai='$nama_pegawai', foto_pegawai='$foto' ,tempat_lahir='$tempat_lahir', tanggal_lahir='$tgl_lahir', jenis_kelamin='$jk', no_hp='$no_hp', agama='$agama', email='$email',alamat='$alamat',gol_darah='$goldarah',status_pernikahan='$stat_nikah', status_kepegawaian='$stat_pegawai', status_user='$stat_user', jabatan='$jabatan', struktural='$struktural', tahun_pensiun='$tahun_pensiun' WHERE nip='$nipAsli' ");
                $lokasi = "../_assets/img/" . $foto;
                $upload = move_uploaded_file($sumber, $lokasi);
                if ($upload) {
                    echo '<script>
